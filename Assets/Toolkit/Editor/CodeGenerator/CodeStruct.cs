...
{
    public class CodeStruct : IStruct
    {
        ...
        public CodeStruct(string name, AccessModifier accessModifier) : this(name) {
            this.accessModifier = accessModifier;
        }

        public CodeStruct(string name, AccessModifier accessModifier, string inherits) : this(name, accessModifier) {
            this.inherits.Add(inherits);
        }

        public CodeStruct(string name, AccessModifier accessModifier, Type inherits) : this(name, accessModifier) {
            this.inherits.Add(inherits.FullName);
        }

        public CodeStruct(string name, AccessModifier accessModifier, IReadOnlyList<string> inherits) : this(name, accessModifier) {
            for(int i = 0, length = inherits.Count; i < length; i++) {
                this.inherits.Add(inherits[i]);
            }
        }

        public CodeStruct(string name, AccessModifier accessModifier, IReadOnlyList<Type> inherits) : this(name, accessModifier) {
            for(int i = 0, length = inherits.Count; i < length; i++) {
                this.inherits.Add(inherits[i].FullName);
            }
        }

        public CodeStruct(AccessModifier accessModifier, string name) {
            this.accessModifier = accessModifier;
            this.name = name;
        }

        public CodeStruct(AccessModifier accessModifier, string name, Type inherits) : this(accessModifier, name, inherits.FullName) { }
        public CodeStruct(AccessModifier accessModifier, string name, string inherits) {
            this.accessModifier = accessModifier;
            this.name = name;
        ...
        public CodeStruct(AccessModifier accessModifier, string name, IReadOnlyList<Type> inherits) : this(accessModifier, name, inherits.Select(x => x.FullName)) { }

        public CodeStruct(AccessModifier accessModifier, string name, IEnumerable<string> inherits) {
            this.accessModifier = accessModifier;
            this.name = name;
        ...
        public CodeStruct(AccessModifier accessModifier, string name, IReadOnlyList<string> inherits) {
            this.accessModifier = accessModifier;
            this.name = name;
            if(inherits != null) {
                for(int i = 0, length = inherits.Count; i < length; i++) {
        ...
        public string GetCode(int indent) {
            ...
            return attRes + $"{GenUtility.Indent(indent)}{GenUtility.Generate(accessModifier)}struct {GenUtility.VerifyName(name)}{InheritanceText} {{{GenUtility.NEWLINE}{GenUtility.Process(codeNodes, indent + 1)}{GenUtility.Indent(indent)}}}{GenUtility.NEWLINE}";
