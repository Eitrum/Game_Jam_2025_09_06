...
{
    [CustomEditor(typeof(WeightedContainerBehaviour))]
    public class WeightedContainerBehaviourEditor : Editor
    {
        private SerializedProperty maxWeightProperty;

        private void OnEnable() {
            maxWeightProperty = serializedObject.FindProperty("maxWeight");
        }

        public override void OnInspectorGUI() {
            var behaviour = target as WeightedContainerBehaviour;
            var container = target as IWeightContainer;
            var items = container.Items;

            using(new EditorGUILayout.VerticalScope("box")) {
                EditorGUILayout.PropertyField(maxWeightProperty);
                EditorGUILayout.LabelField($"{container.CurrentWeight:0.##}kg / {container.MaxWeight:0.##}kg");
